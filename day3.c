...
void secondPart(char **content) {
    ...
    for (int i = 0; i < LINE_LENGTH; i++) {
        ...
        carbonCriteria[i] = !(bool) round((double) carbonColumnSum / positiveCarbonAmount);
        ...
        for (int j = 0; j < LINE_NUMBER; j++) {
            ...
            strncpy(currentCharacter, &content[j][i], 1);
            int currentNumber = atoi(currentCharacter);
            if (currentNumber == oxygenCriteria[i] && (i == 0 || positiveOxygenRows[j] == true)) {
            ...
            if (currentNumber == carbonCriteria[i] && (i == 0 || positiveCarbonRows[j] == true)) {
